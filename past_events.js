...
var pastEventsDate= dataApi.currentDate
// console.log(pastEventsDate)
var dataArray = dataApi.events
// console.log(dataArray)
let array = dataArray.filter(eventocategory => pastEventsDate < eventocategory.date)
// console.log(array)
...
function checkboxPastEvents() { //Creamos la funcion y la nombramos.
...
checkboxPastEvents() //Ejecutamos o llamamos a la funcion.
...
function filterCheckSearch() {//Declaro una funcion, la nombro, pero no aplico parametro porque no lo necesito, que voy a trabajar con parametros externos.
    ...
    pastEventsCards(arrayCombinado)
}
filterCheckSearch()

//CARDS
function pastEventsCards(array) { //Declaro una funcion y la nombro.
    let pastEventsCardsTemplate = "";  //Declaro una variable sin contenido, que cuando se genere el recorrido me vaya guardando los datos. 
    if (array.length !== 0) {
        array.forEach(evento => { //evento hace referencia a cada elemento del array | Al array lo recorro con el forEach(func. ord. sup.) y le asigno un parametro con una funcion flecha que me va a contener los datos de cada card, cuando se recorra
            ...
            cardsIdHtml.innerHTML = pastEventsCardsTemplate;
        })
    } else {
        cardsIdHtml.innerHTML = `<p style="color:white">"You cant´ find what you´re looking for"<p>`
    }
}
//pastEventsCards() //Ejecuto o llamo a la funcion, me lleve adentro de la funcion filterCheckSearch
}
getDataFromApi()